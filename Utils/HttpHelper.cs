...
namespace MobiHymnMaui.Utils
{
	public class HttpHelper
	{
        ...
        public bool SaveHymns(HymnList hymnList)
        {
            ...
            var folder = Directory.CreateDirectory(Path.Combine(rootFolder, folderName));
            var file = File.Create(Path.Combine(folder.FullName, jsonFile));
        ...
        public bool SaveMidi(Stream stream, string fileName)
        {
            var rootFolder = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);

            var folder = Directory.CreateDirectory(Path.Combine(rootFolder, folderName));
            var folderMidi = Directory.CreateDirectory(Path.Combine(folder.FullName, folderMidiName));
            using (var file = File.Create(Path.Combine(folderMidi.FullName, fileName)))
            {
		...
		public bool HymnListFileExists()
		{
            var rootFolder = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            return File.Exists(Path.Combine(rootFolder, folderName, jsonFile));
		}
		...
		public async Task<HymnList> ReadHymns()
		{
            ...
            HymnList hymnList = new HymnList();
            if (HymnListFileExists())
			{
				var settings = await File.ReadAllTextAsync(Path.Combine(rootFolder, folderName, jsonFile));
        ...
        public async Task<bool> DownloadMIDI(string number, CancellationToken cts)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    ret = SaveMidi(fileStream, $"h{number}.mid");
                }
            }
            catch (Exception ex)
            {
