...
namespace MobiHymnMaui.Utils
{
    public class Globals
    {
        ...
        public async Task<bool> DownloadReadHymns(bool forceSync = false)
        {
            ...
            var exists = httpHelper.HymnListFileExists();
        ...
        public async void SaveSettings()
        {
            ...
            try
            {
                ...
                var folder = Directory.CreateDirectory(Path.Combine(rootFolder, folderRootName));
                var file = File.Create(Path.Combine(folder.FullName, settingsName));
        ...
        public async Task<bool> LoadSettings()
        {
            ...
            try
            {
                var fullPath = $"{folderRootName}/{settingsName}";
                var rootFolder = Environment.SpecialFolder.LocalApplicationData;
                    ...
                    foreach (KeyValuePair<string, object> entry in json)
                    {
                        switch(entry.Key)
                        {
                            ...
                            case nameof(ActiveReadTheme):
                                try
                                {
                                    ActiveReadTheme = ((JObject)(entry.Value)).ToColor();
                                }
                                catch (Exception ex)
                                {
        ...
        public static async void ShowToastPopup(string source, string label)
        {
            ...
            await Shell.Current.ShowPopupAsync(toastPopup);
            await new TaskFactory().StartNew(() => { Thread.Sleep(2000); });

            MainThread.BeginInvokeOnMainThread(() => { toastPopup.Close(null); });
        }

        public static async void ShowToastPopup(string source, string label, double size, RectangleF layoutBounds)
        {
            ToastPopup toastPopup = new ToastPopup
            {
                PopupAnim = source,
